...
class therdone extends StatefulWidget {
  const therdone({Key? key, required this.press}) : super(key: key);

  final VoidCallback press;

  @override
  State<therdone> createState() => _therdoneState();
}

class _therdoneState extends State<therdone> {
                                  ...
                                  return Padding(
                                    ...
                                    child: GridView.builder(
                                      ...
                                      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                                        ...
                                        itemCount: shalatProducts.where((product) => product.id == 19 || product.id == 24 ).length,
    itemBuilder: (context, index) {
      final filteredProducts = shalatProducts.where((product) => product.id == 19 ||product.id == 24  ).toList();
      ...
      return almontg2(
        ...
        press: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => hijabviewer(product: product),
            ),
          );
        },
      );
...
class almontg2 extends StatelessWidget {
  const almontg2({
    Key? key,
