...
import 'details.dart'; // Import your Product class
import 'product.dart'; // Import your Product classi




class shalatviewer extends StatefulWidget {
  
  const shalatviewer({Key? key, required this.product}) : super(key: key);
  final Product product;

  @override
  State<shalatviewer> createState() => _shalatviewerState();
}

class _shalatviewerState extends State<shalatviewer> {
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: SingleChildScrollView(
            ...
            dotshifon(product: widget.product,onColorChanged: (newColor) {widget.product.updateSelectedColor(newColor);},),
    
          if ( widget.product.id == 2)
         dotgorget(    product: widget.product, onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor); }, ),

           if (widget.product.id == 3)
          dotcotenkwait(product: widget.product,onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);}),
        
            
        if(widget.product.id == 4) 
        dotglutter( product: widget.product,onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);}),
         
        if(widget.product.id == 5)
        dotcottonliner(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);}),


         if(widget.product.id == 6)
         dotstras(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);}) ,       
      

        if (widget.product.id == 7)
        dotDOUBLE_STRICH(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),
            
          
        if(widget.product.id == 8 )
        dotsatan(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),
           
         if(widget.product.id == 9)
         dotswarih(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),
        
         if(widget.product.id == 10)
         dotshifon_motraz(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),

         if(widget.product.id == 11)
         dotkashmir(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),

         if(widget.product.id == 12)
         dotlikramojaker(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),
          
          if(widget.product.id == 13)
          dotlonen(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),

         if(widget.product.id == 14)
         dotgorgetsatan(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),
         
        if(widget.product.id == 15)
         dotcottonmoskar(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),

                 if(widget.product.id == 16)
         dotlikra_mksar(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),

                          if(widget.product.id == 17)
         dotlyorah_glutter(product: widget.product, // Pass the correct product
         onColorChanged: (newColor) { widget.product.updateSelectedColor(newColor);},),
